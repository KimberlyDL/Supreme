...
class OrderService {
        ...
        try {
            ...
            const order = {
                ...
                items: orderData.items.map((item) => ({
                    ...
                    sale: item.onSale
                        ? {
                            startDate: item.sale?.startDate ? Timestamp.fromMillis(item.sale.startDate) : null,
                            endDate: item.sale?.endDate ? Timestamp.fromMillis(item.sale.endDate) : null,
                            salePrice: item.sale?.salePrice,
                        }
                        : null,
                })),
                totalPrice: orderData.totalPrice || (orderData.items.reduce((total, item) => total + item.totalPrice, 0) - discounts),
            ...
            // Log the activity
            await this.logService.logOrderActivity({
                data: order,
                user: user,
                action: 'Created'
            });
    ...
    async validateSalePrices(items) {
        ...
        if (invalidItems.length > 0) {
            throw new Error(`Some items have invalid sale prices: ${JSON.stringify(invalidItems)}`)
        }
    ...
    async validateOrderItems(items, branchId) {
        ...
        if (stockIssues.length > 0) {
            throw new Error(`Stock validation failed: ${JSON.stringify(stockIssues)}`)
        }
    ...
    async updateOrder(orderData, user) {
        try {
            ...
            // Validate stock availability and sale prices
            await this.validateOrderItems(orderData.items, orderData.branchId || existingOrder.branchId)
            await this.validateSalePrices(orderData.items)
            ...
            const updatedOrder = {
                ...
                totalPrice: orderData.totalPrice || orderData.items.reduce((total, item) => total + item.totalPrice, 0),
...
// Approve an order - this is where inventory is updated
    async approveOrder(orderId, user) {
        try {
            ...
            const branchStockItems = await this._getBranchStockItems(existingOrder.branchId)
            const stockMap = new Map()
            ...
            for (const item of existingOrder.items) {
                ...
                if (!stockItem) {
                    throw new Error(`Stock not found for product ${item.product} (${item.productId})`)
                }
                ...
                if (expirationDates.length === 0) {
                    ...
                    const sortedDates = [...expirationDates].sort((a, b) => a.date - b.date)
                    ...
                    if (remainingToDeduct > 0) {
                        throw new Error(`Not enough stock with expiration dates for ${item.product}`)
                    }
                    ...
                    itemExpirationDates[`${item.productId}-${item.varietyId}`] = datesToDeduct
                }
            }
            ...
            await this.logService.logSaleActivity({
                data: updatedOrder,
                user: user
            })
    ...
    async voidOrder(id, user) {        try {
            ...
            if (existingOrder.status === "Voided" || existingOrder.status === "Returned") {
                throw new Error(`Order is already ${existingOrder.status.toLowerCase()}`)
            }

            // If order is completed, use returnOrder instead
            if (existingOrder.status === "Completed") {
                throw new Error("Cannot void a completed order. Use returnOrder instead.")            }
    ...
    async returnOrder(id, returnReason, user) {
        try {
            ...
            for (const item of existingOrder.items) {
                ...
                const expirationDates = existingOrder.itemExpirationDates?.[itemKey] || []
                ...
                if (expirationDates.length > 0) {
                    ...
                    const expirationTimestamp = Math.floor(expirationDate.getTime() / 1000) // Convert to seconds
    ...
    async deleteOrder(id, user) {
        try {
            ...
            if (existingOrder.status !== "Pending") {
                throw new Error(`Cannot delete order with status: ${existingOrder.status}. Only pending orders can be deleted.`)
            }
    ...
    async _getBranchStockItems(branchId) {
        try {
            ...
            const snapshot = await db.collection("branch_stocks").where("branchId", "==", branchId).get()
