...
export const useProductStore = defineStore('product', {
    ...
    actions: {
        ...
        async setupRealtimeActiveProducts() {
            ...
            try {
                ...
                this.productsForInventory = productdata
                console.log(`Initial fetch complete: ${this.productsForInventory.length} products loaded`)

                // The key fix is in this onSnapshot callback
                this.unsubscribe = onSnapshot(q, async (snapshot) => {
                    console.log(`Product snapshot received with ${snapshot.docChanges().length} changes`)
                    ...
                    const changePromises = snapshot.docChanges().map(async (change) => {
                        console.log(`Processing ${change.type} for product ${change.doc.id}`)
                    ...
                    if (hasChanges) {
                        console.log(`Updating products array with ${updatedProducts.length} items`)
        ...
        stopListening() {
            if (this.unsubscribe) {
                ...
                if (this.productsForInventory)
                    this.productsForInventory = [];
            }
        },
    ...
    persist: [
        ...
        {
            key: 'product-session',
            paths: ['products', 'loading', 'error', 'lastVisible', 'hasMore', 'batchSize', 'filters'],
