    ...
    <main class="flex-1 md:ml-0">
      ...
      <div :class="[
        ...
        <div v-for="product in productStore.products" :key="product.id" @click="clickProduct(product)"
          class="relative z-10 bg-white rounded-lg shadow-md cursor-pointer transition duration-200 transform hover:shadow-xl overflow-hidden">

          <!-- Sale badge -->
          <div v-if="product.sale && product.sale.onSale" class="absolute top-0 right-0 z-20">
            <div v-if="isSaleActive(product.sale)"
              class="bg-red-500 text-white px-3 py-1 font-semibold text-sm rounded-bl-lg">
              SALE
            </div>
            <div v-else-if="isSaleUpcoming(product.sale)"
              class="bg-yellow-400 text-gray-800 px-3 py-1 font-semibold text-sm rounded-bl-lg">
              SALE SOON
            </div>
          </div>
          ...
          <div v-if="product.stockQuantity !== undefined && product.stockQuantity <= 10"
            class="absolute top-0 left-0 z-20 bg-orange-500 text-white px-3 py-1 font-semibold text-sm rounded-br-lg">
            LOW STOCK
          </div>
          ...
          <div class="relative rounded-t-lg overflow-hidden">
            ...
            <!-- Placeholder for "Items Sold" badge - to be implemented when data is available -->
            <!-- <div class="absolute bottom-2 left-2 bg-gray-800 bg-opacity-75 text-white px-2 py-1 rounded-lg text-xs">
              Sold: 42
            </div> -->
          </div>
          ...
          <!-- Product info -->
          <div class="p-4">
            <!-- Categories as small pills -->
            <div v-if="product.category && product.category.length > 0" class="flex flex-wrap gap-1 mb-2">
            ...
            <h2 class="text-lg font-semibold line-clamp-1"> {{ product.name }}</h2>
            <p v - if="product.description" class="text-sm text-gray-500 line-clamp-2 mt-1 mb-2">
              {{ product.description }}</p>

            <div class="flex justify-between items-center mt-auto">
              <div class="flex flex-col">
                <div v-if="product.sale && product.sale.onSale && isSaleActive(product.sale)" class="flex items-center">
                  <span class="text-lg font-bold text-red-600"> ${{ product.sale.salePrice.toFixed(2) }}</span>
                  <span class="ml-2 text-sm line-through text-gray-500">${{ product.basePrice.toFixed(2) }}</span>
                </div>
                <div v-else>
                  <span class="text-lg font-bold text-gray-900">${{ product.basePrice.toFixed(2) }}</span>
                </div>
                <!--Sale countdown for upcoming sales-->
                <div v -if="product.sale && product.sale.onSale && isSaleUpcoming(product.sale)"
                  class="text-xs text-yellow-600">
                  Sale in {{ getDaysUntilSale(product.sale) }} days
                </div>
              </div>

              <!-- Menu button -->
              <div class="relative z-50">
                <button :id="`dropdownButton-${product.id}`" type="button"
                ...
                <div :id="`dropdown-${product.id}`"
                  ...
                  <ul class="py-2 text-sm text-gray-700 dark:text-gray-200">
                    ...
                    <li>
                      <button @click.stop="confirmDelete(product.id)"
                        class="block w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                        Remove
                      </button>
                    </li>
...
/* Optional: Add this animation for the sale badge */
@keyframes pulse {
