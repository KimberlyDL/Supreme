    ...
    orderData.value = {
      ...
      items: order.items.map(item => {
        ...
        return {
          ...
          unitPrice: item.unitPrice || (item.variety ? item.variety.varietyPrice : 0), // Ensure unitPrice is set
          ...
          variety: item.variety ? {
            ...
            varietyName: item.variety.varietyName, // Keep for backend compatibility
            varietyQuantity: item.variety.varietyQuantity, // Keep for backend compatibility
            varietyPrice: item.variety.varietyPrice, // Keep for backend compatibility
...
const addToOrder = (product) => {
  ...
  if (existingItemIndex !== -1) {
    ...
    orderData.value.items.push({
      ...
      quantity,
      pricePerUnit,
      unitPrice: pricePerUnit, // Ensure unitPrice is set
      maxQuantity: variety.stockQuantity,
      variety: {
        name: variety.name,
        unit: variety.unit,
        quantity: variety.quantity,
        varietyName: variety.name, // Add this for backend compatibility
        varietyQuantity: variety.quantity, // Add this for backend compatibility
        varietyPrice: pricePerUnit, // Add this for backend compatibility
...
const updateOrder = async () => {
  try {
    ...
    // Ensure all items have unitPrice set
    const itemsWithUnitPrice = orderData.value.items.map(item => ({
      ...item,
      unitPrice: item.pricePerUnit // Ensure unitPrice is set
    }));
