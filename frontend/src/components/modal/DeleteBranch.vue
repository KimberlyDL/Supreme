<template>
    <div class="p-6 max-w-md w-full bg-white rounded-lg">
      ...
      <div class="flex justify-end gap-4">
        ...
        <button 
          @click="confirmDelete" 
          :disabled="userConfirmation !== confirmationText || isSubmitting"
  ...
  const deleteBranch = async () => {
    try {
      ...
      // Check if the error is due to branch having operations
      if (error.response?.data?.message?.includes('operations exist')) {
        alert('This branch cannot be deleted because it has associated operations. It has been marked as inactive instead.');
      } else {
